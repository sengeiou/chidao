...
@interface InVideoViewController ()<AVCaptureFileOutputRecordingDelegate,UIAlertViewDelegate>//视频文件输出代理
...
@implementation InVideoViewController{
    ...
    dispatch_source_t _timer; //计时器10秒
    dispatch_source_t _timer2; //计时器3秒
    
    UIView* progressPreView; //进度条
    float progressStep; //进度条每次变短的最小单位
    float progressStep2; //进度条每次变短的最小单位
...
@synthesize totalTime,currentTime,totalTime2,currentTime2;
...
- (void)viewDidLoad {
    ...
    [[APPConfig getInstance] setInFlag:@"0"];
    ...
    progressStep2 =SCREEN_WIDTH/totalTime2;
    ...
    NSURLSessionDataTask *task = [[NewsClient sharedClient] getLightNums:OBULightURL withToken:@"3090fcceb96b4f85a6e029329520563c" withObuID:@"3701081320651001" completion:^(NSMutableDictionary *dic, NSError *error) {
        ...
        if (!error) {
            ...
            if ([[dic objectForKey:@"code"] isEqualToString:@"000000"]) {
                ...
                lightIn = [[[dic objectForKey:@"data"] objectForKey:@"in_count"] intValue];
                lightOut =[[[dic objectForKey:@"data"] objectForKey:@"out_count"] intValue];
...
-(void)stopTimer{
    totalTime=10;
    currentTime=10;
    [[APPConfig getInstance] setInFlag:@"1"];
    [progressPreView setFrame:CGRectMake(0, preLayerHeight, myScreenWidth, 4)];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    dispatch_source_set_event_handler(_timer, ^()
    {
        dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [[WJObuSDK sharedObuSDK] qryReaderStates:^(BOOL status, id data, NSString *errorMsg) {
                    ...
                    switch ([data intValue]) {
                        case 00:{
                            ...
                            dispatch_source_set_timer(_timer2, dispatch_time(DISPATCH_TIME_NOW, 0), interval, 0);
                            //设置回调
                            dispatch_source_set_event_handler(_timer2, ^{
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    currentTime2 = totalTime2--;
                                    if (currentTime2==0) {
                                        dispatch_suspend(_timer2);
                                        
                                        dispatch_async(dispatch_get_global_queue(0, 0), ^{
                                            
                                            [[WJObuSDK sharedObuSDK] sendLightNums:lightIn index:5 callback:^(BOOL status, id data, NSString *errorMsg) {
                                    ...
                                    float progressWidth = progressPreView.frame.size.width-progressStep2;
                                    [progressPreView setFrame:CGRectMake(0, preLayerHeight, progressWidth, 4)];
                                    alertLabel.text = [NSString stringWithFormat:@"(%i)秒后闪灯",(int)currentTime2];
            ...
            [progressPreView setFrame:CGRectMake(0, preLayerHeight, progressWidth, 4)];
            alertLabel.text = [NSString stringWithFormat:@"请尽快安装OBU(%i)",(int)currentTime];
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if ([APPConfig getInstance].currentFlag&&[[APPConfig getInstance].currentFlag isEqualToString:@"1"]) {
        ...
        if ([APPConfig getInstance].inFlag&&[[APPConfig getInstance].inFlag isEqualToString:@"1"]) {
    ...
    [exporter exportAsynchronouslyWithCompletionHandler:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(exporter.status == AVAssetExportSessionStatusCompleted){
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"车内视频录制完毕,请拍摄车外视频"delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alert show];
...
- (void)alertView:(UIAlertView*)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    outVC.lightIn2 = lightIn;
    outVC.lightOut2 = lightOut;
...
- (NSString *)getVideoMergeFilePathString
{
    ...
    if ([manager fileExistsAtPath:path]){
        fileName = [[path stringByAppendingPathComponent:@"ios_in"] stringByAppendingString:@".mp4"];
    }
...
- (NSString *)getVideoSaveFilePathString
{
    ...
    NSString *fileName;
    if ([manager fileExistsAtPath:path]){
        fileName = [[path stringByAppendingPathComponent:@"ios_in"] stringByAppendingString:@".mov"];
    }
