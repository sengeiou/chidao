...
@interface AddAddressViewController ()<UITableViewDataSource,UITableViewDelegate,UIPickerViewDataSource, UIPickerViewDelegate>{
...
- (void)saveClick{
    ...
    if ([cell1.tf.text isEqualToString:@""]) {
        ...
        NSURLSessionDataTask *task = [[NewsClient sharedClient] addAddress:AddAddrURL withToken:[APPConfig getInstance].tokenWord withName:cell1.tf.text withAddressArea:cell3.detailTextLabel.text withAddrDetail:cell4.tf.text withPostid:cell5.tf.text withMobile:cell2.tf.text  completion:^(NSMutableDictionary *dic, NSError *error) {
